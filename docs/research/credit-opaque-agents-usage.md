...
## 九、2026-09-05 第四次实测:WorkBuddy 全量复测(数据增长 40 倍 + 混元免费 + 逐模型费率)

触发:用户问「WorkBuddy 中有具体的数据吗」→ 对 `~/.workbuddy` 全量只读扫描
(28 个 `projects/*/*.jsonl` + `workbuddy.db`),验证 §6.3 取证在数据量增长后是否仍成立。

### 9.1 数据规模(对照 §6.3 的两会话快照)

- 28 个会话(2026-08-05 ~ 2026-09-01,横跨 zlens / ardot-agent-kit / deer-flow / WorkBuddy 自身对话);
  `workbuddy.db` 2026-09-03 仍在更新(sessions 28 行、session_usage 26 行)。
- 822 条 `type=message, role=assistant` 记录,其中 **160 条带 `providerData.rawUsage`**(其余 662 条为
  无用量载荷的辅助/状态消息,无计费含义);**credit 字段 160/160 全部在值**——包括值=0.00 的行
  (见 9.2),「缺失(null)vs 0」的纪律在 WorkBuddy 同样必要且可行。
- 合计:total_tokens **33,836,724**、prompt 33,687,003(其中 cache_hit 32,150,889 = prompt 的 95.4%)、
  completion 149,721、reasoning 46,920、cache_write 0;**Σ credit = 325.42**。
- **无 original/discount 字段**:WorkBuddy 没有「原价/实扣」两笔——§8.3 的 original_credits
  逐条保留纪律是 Qoder CN 专属,不适用于它。
- 对照 §6.3 的 7.65 积分/6.7 万 token:三周增长约 40 倍——WorkBuddy 是本机活账本,不是残留数据。
  §6.3 的两会话(auto→glm-5.2,66,771 tok / 7.65 cr)完整包含在本轮聚合中,数字逐项吻合。

### 9.2 逐渠道明细(本轮决定性发现)

| 渠道(requestModelId→model) | msgs | credit 合计 | total_tokens | 折合 tok/credit |
|---|---|---|---|---|
| hy4-preview-x→hy4-preview-x | 70 | 222.41 | 20,564,080 | ≈ 92,500 |
| kimi-k3-1→kimi-k3-1 | 9 | 82.61 | 1,359,493 | ≈ 16,500 |
| auto→glm-5.2 | 2 | 7.65 | 66,771 | **≈ 8,728** |
| auto→glm-5.3 | 1 | 4.70 | 37,737 | ≈ 8,030 |
| glm-5.2→glm-5.2(直选) | 1 | 2.69 | 38,488 | ≈ 14,300 |
| glm-5.3-flash→glm-5.3-flash | 4 | 5.36 | 1,140,825 | ≈ 212,800 |
| **hy4-preview→hy4-preview** | 67 | **0.00** | **10,331,744** | 免费 |
| **hy3→hy3** | 6 | **0.00** | **297,586** | 免费 |

- **混元(hy)系列免费**:hy3 / hy4-preview 合计约 1063 万 token、credit 恒为 0(字段在值、值为 0)——
  腾讯自家模型零积分引流。标价折算(路径 E)对 0 积分行 = 0,语义安全;**token×单价口径则会给这些行
  算出假钱** → 「三笔账两两不相加」(规格 D3)再添一个实证。
- **无全局系数**:tok/credit 跨模型相差一个数量级(约 8 千 ~ 21 万)。§6.3 的「1 credit ≈ 8.7K token」
  在 glm-5.2 通道三点收敛(8,756 / 8,701 / 8,728),**但它只是 glm-5.2 的通道常数,不是产品常数**;
  反向标定(路径 C)只能逐通道做,样本最小的通道(kimi 9 条)也仅够方向判断。
- **同真实模型、不同档位,费率方向性不同**:glm-5.2 直选 ≈14,300 tok/cr vs auto→glm-5.2 ≈8,728
  (各仅 1~2 条消息、缓存构成不同,只作方向提示)——`workbuddy|<档位>|<真实模型>` 三元组跨渠道
  独立计价(D5)再获佐证,任何「按真实模型合并取价」的捷径都不可走。
- **auto 档下真实模型不唯一**:auto→glm-5.2 与 auto→glm-5.3 并存 → 同档位多真实模型必须分行。

### 9.3 对规格与票的含义

- D5 WorkBuddy 适配器设计不变;本轮未逐条复算 §6.3 恒等式(只做全量聚合),票实现时的
  逐条恒等式断言与会话级 `credit_json == Σ rawUsage.credit` 校验(§6.3/D5)照旧保留。
- 路径 C 结论收窄为「逐通道可标定、跨通道不可标定」;「系数不进钱管线、标价由用户按官网对账录入」
  (路径 E + 规格 D3)维持不变。
- `credit=0` 与 `credit` 缺失在本机数据中都真实出现 → fixture 必须同时覆盖两种形状
  (hy 免费行=0,异常行=缺失),适配器按规格 D1 的 null-vs-0 纪律区分。
