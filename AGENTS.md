...
│   ├── core/               # config.py(配置)、cost.py(成本估算)、rates.py(系数快照)、stats.py(分位数)
│   ├── sources/            # base.py 协议 + zcode/minimax/opencode/workbuddy/qoder_cn/qoder/claude/multi 适配器,只读
...
- **token 四档互斥**:适配器交出的 `input/output/cache_creation/cache_read` 必须互斥且
  ...
  积分计费来源(Qoder CN 等)本地 token 恒为 0,必须以 `tokens_reported=False` 声明——
  那些占位 0 禁止参与逐档乘价,界面显示「未提供」而不是 0。
...
- **三笔钱不合并**:「按量消耗」= Σ(单价 × 用量),**未计价渠道按 ¥0 计入**(产品决策
  ...
  **留空(null)与填 0 必须可区分**——把「没填」显示成 `¥0.00` 就是撒谎。价格表页的「现总价」
  ...
  (v3 的 `discount_credits` 同样是纯派生展示值)。
- **凭据与密文字段禁读**:适配器与快照抓取只取白名单计价字段;任何 `secret://` 前缀 KV、
  键名/路径命中 `ak/sk/api_key/secret/base_url/auth_type/jwt/token` 的字段、终端状态缓冲
  一律**不读、不解密、不回显**(v3 系数快照的安全边界,测试断言级)。
