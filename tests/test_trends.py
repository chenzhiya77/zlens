...
def _rows():
    # ZCode stores the cached prefix inside input_tokens (total = input + output),
    # so a billable 100 with 1000 cached is stored as input_tokens = 1100.
    return [
        {
            "model_id": "alpha",
...
def test_daily_trends_fold_and_cost_rule(client_factory, tmp_path):
    ...
    table = PriceTable.load(
        _pricing(tmp_path, {"zcode|unknown|alpha": {"input": 3.0, "output": 0.0}})
    )
    ...
    assert body.by_model[0].estimated_cost is None
    assert body.by_model[1].estimated_cost == 0.0003
